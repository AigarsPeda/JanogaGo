...
<header class="site-header">
	...
	<nav class="site-nav" aria-label="Primary navigation"><?php wp_nav_menu( array( 'theme_location' => 'primary', 'container' => false, 'menu_class' => 'jg-menu', 'fallback_cb' => 'jg_fallback_menu', 'depth' => 1 ) ); ?><?php if ( function_exists( 'pll_the_languages' ) ) { $languages = pll_the_languages( array( 'raw' => 1, 'hide_current' => 0 ) ); if ( is_array( $languages ) ) { echo '<ul class="language-switcher">'; foreach ( $languages as $language ) { echo '<li' . ( ! empty( $language['current_lang'] ) ? ' class="current-lang"' : '' ) . '><a lang="' . esc_attr( $language['slug'] ) . '" href="' . esc_url( $language['url'] ) . '">' . esc_html( strtoupper( $language['slug'] ) ) . '</a></li>'; } echo '</ul>'; } } ?><?php if ( $primary_cta['label'] && $primary_cta['url'] ) { ?><a class="nav-cta" href="<?php echo esc_url( $primary_cta['url'] ); ?>"><?php echo esc_html( $primary_cta['label'] ); ?></a><?php } ?>
	</nav>
</header>
